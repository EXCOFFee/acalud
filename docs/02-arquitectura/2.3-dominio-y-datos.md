...
| **Versión** | 0.1.1 · **Fecha:** 2026-07-09 · **Estado:** 🟡 Borrador |
...
- `eventos_auditoria` y `movimientos_stock` (kardex): **append-only total** (sin UPDATE ni
  DELETE). Se aplica hoy por **trigger** (portátil y verificable contra Testcontainers) y en el
  deploy (Etapa 1) se refuerza con **REVOKE de permisos** sobre un rol de runtime de privilegio
  mínimo — defensa en profundidad (NFR-S6).
- `outbox_emails`: **sin DELETE**; el worker del outbox (CU-E05) **sí** puede hacer UPDATE, pero
  solo de `estado`/`intentos`/`procesado_en`. No es append-only total: lo sería a costa de
  romper el reintento idempotente del worker.
...
| Append-only total en auditoría y kardex (sin UPDATE/DELETE: trigger + REVOKE de rol en E1); `outbox_emails` sin DELETE, con UPDATE de estado/intentos/procesado_en por el worker | NFR-S6, integridad del kardex, idempotencia del worker (CU-E05) |
...
| 0.1.1 | 2026-07-09 | Enmienda §6 append-only (alinea doc↔código de la migración 0001, tarea 0.2): auditoría y kardex append-only total (trigger hoy + REVOKE de rol en E1); `outbox_emails` sin DELETE pero con UPDATE de estado/intentos/procesado_en por el worker (resuelve la contradicción con CU-E05) |
