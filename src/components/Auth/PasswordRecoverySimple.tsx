  ...
  const requestPasswordReset = async (): Promise<void> => {
    ...
    try {
      ...
      if (response.ok && data.success) {
        setSuccessMessage(
          'Se ha enviado un enlace de recuperación a tu email. Revisa tu bandeja de entrada y carpeta de spam.'
        );
  ...
  const validateToken = async (tokenToValidate: string): Promise<void> => {
    ...
    try {
      ...
      if (response.ok && data.success) {
        ...
        setSuccessMessage('Token válido. Puedes establecer tu nueva contraseña.');
      } else {
